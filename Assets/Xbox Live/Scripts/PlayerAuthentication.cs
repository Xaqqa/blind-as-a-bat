...
namespace Microsoft.Xbox.Services.Client
{
    public class PlayerAuthentication : MonoBehaviour
    {
        ...
        {
            if (mainmenu.playerAuthXbox)
            {
                SignIn();
                mainmenu.playerAuthXbox = false;
            }

            if (mainmenu.checkSignedIn)
            {
                if (this.xboxLiveUser != null && this.xboxLiveUser.IsSignedIn)
                {
                    mainmenu.confirmSignedIn = true;
                }
                else
                {
                    mainmenu.confirmSignedIn = false;
                }
            }
        }
        ...
        {
            loginText.text = "Logged in as " + this.xboxLiveUser.Gamertag;
